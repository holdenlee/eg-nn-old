...
from cifar.cifar_utils import *
from cifar.cifar_model_mwu import *
...
def main(_):
    train_data, test_data = get_bf_cifar(augmentation=True, batch_size=FLAGS.batch_size, normalize=True)
    #FLAGS.label_smooth, FLAGS.testing
    sess = tf.Session() #start_keras_session(1)
    model = lambda x: cifar_model_mwu(x,us=[20,20,20,20,20,20], weights=False)
    ...
    opter= lambda gs: MWOptimizer(tf.get_collection('Ws'), tf.get_collection('bs'), learning_rate=tf.train.exponential_decay(FLAGS.learning_rate, gs, 1000, 0.99, staircase=True))
                                  #FLAGS.learning_rate/tf.ceil(tf.cast(gs+1, tf.float32)/500), smoothing=0)
    ...
    addons = [GlobalStep(),
              ...
              Train(opter, FLAGS.batch_size, train_feed = {}, loss = 'loss', print_steps = FLAGS.print_steps),
