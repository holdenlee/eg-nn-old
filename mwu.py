...
class MWOptimizer:
    ...
    def apply_gradients(self, gradients):
        ...
        return make_mw_ops_with_biases(self.ws, self.bs, self.eta, gradients=gradients, smoothing=self.smoothing)

def make_mw_ops_with_biases(ws, bs, eta, y=None, gradients=None, smoothing=0): #include clipping
    # https://www.tensorflow.org/api_docs/python/tf/gradients
    if gradients==None:
        grads = tf.gradients(y, ws)
...
def mnist_conv(x,y, u=50,u2=50):
    ...
    yf = tf.contrib.layers.flatten(y2)
...
if __name__=='__main__':
    ...
    ans = fors_zip([[10, 20, 50], [10, 20, 50], [0.3, 0.1, 0.03, 0.01]],
        #[[1, 3, 10, 20 ,50],[1, 3, 10, 20 ,50],[0.1]],
        #[[1, 3, 10, 20 ,50],[1, 3, 10, 20 ,50],[0.1]], 
                   lambda li: mnist_conv_test(li[0], li[1], li[2], 3000, smoothing=0))
#[[10,20,50,80], [0.001,0.003,0.01,0.03,0.1,0.3,1]]
    print(ans)
